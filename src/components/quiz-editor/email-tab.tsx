    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="rounded-lg border bg-card p-5 space-y-3">
        ...
        <div className="text-xs rounded-md border bg-muted/30 p-2">
          <div className="font-medium mb-1">Merge fields</div>
          <p className="text-muted-foreground mb-1">Use these tokens in the subject, intro, or body. They'll be replaced with the quiz taker's values.</p>
          <div className="flex flex-wrap gap-1 font-mono">
            {["{{first_name}}","{{last_name}}","{{name}}","{{email}}","{{company}}","{{score}}","{{percentage}}","{{points}}","{{tier}}","{{quiz_title}}"].map((t) => (
              <code key={t} className="px-1.5 py-0.5 rounded bg-background border">{t}</code>
            ))}
          </div>
        </div>
