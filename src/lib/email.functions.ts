...
export const sendQuizResult = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const html = renderResultEmail({
      ...
      recipientName: sub.prospect_name || "there",
      score: Number(sub.total_score),
      max: Number(sub.max_score),
      pct: Number(sub.percentage),
      tierName: tier?.name || "Your result",
      intro: tier?.email_intro || "Thanks for completing the assessment. Here's a summary of your results.",
      body: tier?.email_body || tier?.description || "",
      ctaUrl,
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: tier?.email_subject || `Your ${quiz.title} results`,
        html,
