...
function PublicQuiz() {
  ...
  const [lead, setLead] = useState({ firstName: "", lastName: "", email: "", company: "", custom: {} as Record<string, string> });
  ...
  const submitAnswers = async () => {
    ...
    try {
      ...
      const { data: sub, error: subErr } = await supabase
        ...
        .insert({
          quiz_id: quiz.id,
          prospect_name: `${lead.firstName} ${lead.lastName}`.trim(),
          prospect_first_name: lead.firstName || null,
          prospect_last_name: lead.lastName || null,
        ...
        {stage === "lead" && (
          <form onSubmit={submitLead} className="space-y-4 max-w-md mx-auto">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><Label>First name</Label><Input required value={lead.firstName} onChange={(e) => setLead({ ...lead, firstName: e.target.value })} /></div>
              <div><Label>Last name</Label><Input required value={lead.lastName} onChange={(e) => setLead({ ...lead, lastName: e.target.value })} /></div>
            </div>
