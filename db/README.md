...
## 0010_prospect_name_split.sql

Adds `submissions.prospect_first_name` and `submissions.prospect_last_name`
so the public quiz form can collect them as separate fields and so the
result email can use them as merge tokens (e.g. `{{first_name}}`). The
existing `prospect_name` column is kept and populated as
`first last` for back-compat. Apply once in the SQL editor — safe to re-run.
